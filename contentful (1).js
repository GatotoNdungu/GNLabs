...
window.GNContentful = (function () {
  ...
  // Contentful returns embedded Rich Text media/entries as unresolved
  // Link objects ({ sys: { id, linkType } }) — the actual asset or entry
  // fields live separately, in the response's top-level `includes` section.
  // These two maps let the renderer look a link up by id and get the real
  // fields, which is the step that was missing before (embedded images were
  // rendering as nothing because the link was never cross-referenced).
  function mapAssets(data) {
    const assetMap = {};
    ((data.includes && data.includes.Asset) || []).forEach(a => {
      const url = (a.fields && a.fields.file && a.fields.file.url) ? 'https:' + a.fields.file.url : null;
      assetMap[a.sys.id] = { url, title: (a.fields && a.fields.title) || '', description: (a.fields && a.fields.description) || '' };
    });
    return assetMap;
  }
  ...
  function mapEntries(data) {
    const entryMap = {};
    ((data.includes && data.includes.Entry) || []).forEach(e => {
      entryMap[e.sys.id] = { contentType: e.sys.contentType && e.sys.contentType.sys.id, fields: e.fields || {} };
    });
    return entryMap;
  }

  function mapEntry(item, assetMap, entryMap) {
    const f = item.fields || {};
    const imgId = f.coverImage && f.coverImage.sys && f.coverImage.sys.id;
    const coverAsset = imgId ? assetMap[imgId] : null;
    return {
      id: item.sys.id,
      ...
      image: (coverAsset && coverAsset.url) || null,
      ...
      // Needed by renderRichText() to resolve embedded assets/entries inside
      // `body` — pass these straight through, e.g.
      // renderRichText(article.body, article.assetMap, article.entryMap)
      assetMap,
      entryMap
    };
  }
  ...
  async function fetchArticles({ force = false } = {}) {
    ...
    try {
      const data = await cdaFetch(`content_type=${CONTENT_TYPE}&order=-fields.publishDate&limit=100&include=2`);
      if (!data.items) return [];
      const assetMap = mapAssets(data);
      const entryMap = mapEntries(data);
      const articles = data.items.map(item => mapEntry(item, assetMap, entryMap));
  ...
  async function fetchArticleBySlug(slug, { force = false } = {}) {
    ...
    try {
      const data = await cdaFetch(`content_type=${CONTENT_TYPE}&fields.slug=${encodeURIComponent(slug)}&limit=1&include=2`);
      if (!data.items || !data.items.length) return null;
      const assetMap = mapAssets(data);
      const entryMap = mapEntries(data);
      const article = mapEntry(data.items[0], assetMap, entryMap);
  ...
  //
  // Call this as renderRichText(article.body, article.assetMap, article.entryMap)
  // — both maps come back attached to whatever fetchArticles()/
  // fetchArticleBySlug() returned, so you don't need to build them yourself.
  // ==========================================================================

  function renderRichText(doc, assetMap, entryMap) {
    if (!doc || !doc.content) return '';
    return doc.content.map(n => renderNode(n, assetMap, entryMap)).join('');
  }
  ...
  function renderNode(node, assetMap, entryMap) {
    switch (node.nodeType) {
      ...
      case 'blockquote':
        return `<blockquote>${(node.content || []).map(n => renderNode(n, assetMap, entryMap)).join('')}</blockquote>`;
      case 'hr':
        return '<hr>';
      case 'embedded-asset-block':
        return renderEmbeddedAsset(node, assetMap);
      case 'embedded-entry-block':
        return renderEmbeddedEntry(node, entryMap);
      default:
  ...
  // node.data.target here is an UNRESOLVED link — { sys: { id, linkType:
  // 'Asset' } } — not the asset itself. The real fields live in assetMap,
  // keyed by that same id, built from the response's top-level `includes`.
  function renderEmbeddedAsset(node, assetMap) {
    const linkId = node.data && node.data.target && node.data.target.sys && node.data.target.sys.id;
    const asset = linkId && assetMap ? assetMap[linkId] : null;
    if (!asset || !asset.url) {
      console.warn(`[GN Labs] Embedded asset (id: ${linkId}) has no matching entry in assetMap — is it published, and was the fetch made with a sufficient include= depth?`);
      return '';
    }
    const alt = asset.description || asset.title || '';
    return `<figure class="article-figure"><img src="${escapeHtml(asset.url)}" alt="${escapeHtml(alt)}" loading="lazy">${alt ? `<figcaption>${escapeHtml(alt)}</figcaption>` : ''}</figure>`;
  }

  // Same story as above: node.data.target is an unresolved link to an
  // Entry, resolved here against entryMap. Add one branch per custom
  // embeddable content type you create in Contentful. Field names below
  // (label/code, label/text, text) are a starting point — match them to
  // whatever you actually name the fields.
  function renderEmbeddedEntry(node, entryMap) {
    const linkId = node.data && node.data.target && node.data.target.sys && node.data.target.sys.id;
    const entry = linkId && entryMap ? entryMap[linkId] : null;
    if (!entry) {
      console.warn(`[GN Labs] Embedded entry (id: ${linkId}) has no matching entry in entryMap — is it published?`);
      return '';
    }
